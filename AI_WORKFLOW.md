...
*   **API Rate Limit Resilience (Manual Intervention):** When testing on the live Vercel deployment, the user hit a Google API 503 Capacity error. The user directed the AI to engineer a robust cascading fallback system inside the extraction route that retries `gemini-3.5-flash` before gracefully falling back to `gemini-1.5-flash`, demonstrating strong operational production thinking.
